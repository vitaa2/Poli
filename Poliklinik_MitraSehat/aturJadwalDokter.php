    ...
    if (isset($_POST['id'])) {  
        ...
        if ($statues == 1) {  
            ...
            if ($activeCount > 0) {  
                ...
                $stmt_update = $mysqli->prepare("UPDATE jadwal_periksa SET statues=0 WHERE id_dokter=? AND statues=1 AND id!=?");  
                $stmt_update->bind_param("ii", $id_dokter, $id);  
                $stmt_update->execute();  
                $stmt_update->close();  
            }  
        }  
        ...
        // --> Tambahkan pengecekan untuk memastikan hari belum ada  
        $stmt_check_hari = $mysqli->prepare("SELECT COUNT(*) FROM jadwal_periksa WHERE id_dokter=? AND hari=?");  
        $stmt_check_hari->bind_param("is", $id_dokter, $hari);  
        $stmt_check_hari->execute();  
        $stmt_check_hari->bind_result($hariCount);  
        $stmt_check_hari->fetch();  
        $stmt_check_hari->close();  

        if ($hariCount > 0) {  
            echo "  
                <script>  
                    alert('Gagal! Hari ini sudah terdaftar. Silakan pilih hari lain.');  
                </script>  
            ";  
            return; // Hentikan eksekusi jika hari sudah ada  
        }  
        ...
        if ($statues == 1) {  
            ...
            if ($activeCount > 0) {  
                // Nonaktifkan jadwal aktif lainnya  
                $stmt_update = $mysqli->prepare("UPDATE jadwal_periksa SET statues=0 WHERE id_dokter=? AND statues=1");  
